...
module Dragonfly
  ...
  class Content
    ...
    def shell_eval(opts={})
      ...
      command = yield(should_escape ? Shellwords.escape(path) : path)
    ...
    def shell_generate(opts={})
      ...
      new_path = should_escape ? Shellwords.escape(tempfile.path) : tempfile.path
    ...
    def shell_update(opts={})
      ...
      should_escape = opts[:escape] != false
      tempfile = Utils.new_tempfile(ext)
      old_path = should_escape ? Shellwords.escape(path) : path
      new_path = should_escape ? Shellwords.escape(tempfile.path) : tempfile.path
